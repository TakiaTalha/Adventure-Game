...
public class Room implements Serializable {
     ...
     */
    public Room(String roomName, int roomNumber, String roomDescription, String troll){
        this.roomName = roomName;
        this.roomNumber = roomNumber;
        this.roomDescription = roomDescription;
        this.isVisited = false;
        // Use troll factory to create the troll for the room
        TrollFactory trollFactory = new TrollFactory();
        if (!troll.equals("-----")) {
            if (troll.equals("MINITROLL")) {
                this.troll = trollFactory.createTroll(TrollType.MINI);
            } else if (troll.equals("SUPERTROLL")) {
                this.troll = trollFactory.createTroll(TrollType.SUPER);
            } else if (troll.equals("KINGTROLL")) {
                this.troll = trollFactory.createTroll(TrollType.KING);
            }
        }
    }
    ...
    public String getObjectString() {
        ...
        for (int i=0;i<objectsInRoom.size();i++){
            String x = objectsInRoom.get(i).getDescription();
            list.add(x);
        }
    ...
    public String getCommands() {
        List list = new ArrayList();
        for (int i=0;i<motionTable.getDirection().size();i++){
            String x = motionTable.getDirection().get(i).getDirection().toUpperCase();
            if (!list.contains(x)){
                list.add(x);
            }
        }
